-module(swag_server_apikeys_logic_handler).

-export([handle_request/4]).
-export([map_error/2]).

-type operation_id()    :: swag_server_apikeys:operation_id().
-type api_key()         :: swag_server_apikeys:api_key().
-type auth_context()    :: swag_server_apikeys:auth_context().
-type object()          :: swag_server_apikeys:object().
-type request_context() :: swag_server_apikeys:request_context().
-type handler_opts(T)   :: swag_server_apikeys:handler_opts(T).
-type logic_handler(T)  :: swag_server_apikeys:logic_handler(T).
-type response()        :: swag_server_apikeys:response().

-type validation_error() :: swag_server_apikeys_validation:error().
-type error_type()      :: validation_error.
-type error_message()   :: swag_server_apikeys:error_reason().
