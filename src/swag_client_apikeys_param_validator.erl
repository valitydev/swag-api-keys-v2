%% -*- mode: erlang -*-
-module(swag_client_apikeys_param_validator).

-export([validate/2]).

-type param_base_rule() ::
    swag_client_apikeys_common_validator:param_rule() |
    {enum, [atom()]} |
...
-spec validate(Rule :: param_rule(), Param :: swag_client_apikeys:value()) ->
    ok | {ok, Prepared :: swag_client_apikeys:value()} | error.
...
validate({enum, Values}, Value) ->
    try
        FormattedValue = swag_client_apikeys_utils:binary_to_existing_atom(Value, utf8),
...
validate({RuleName, _} = Rule, Value) when
    ...
    RuleName =:= format
->
    swag_client_apikeys_common_validator:validate(Rule, Value).
